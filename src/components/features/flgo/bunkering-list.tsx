...
import { IconPlus, IconPencil, IconDroplet, IconFilter, IconX } from "@tabler/icons-react";
import { useFlgoStore } from "@/store/flgo-store";

interface BunkeringListProps {
  onNew:  () => void;
...
// ── Component ─────────────────────────────────────────────────────────────────
export default function BunkeringList({ onNew, onEdit }: BunkeringListProps) {
  ...
  const uniqueFuelTypes = useMemo(
    () => [...new Set(bunkerings.flatMap((r) => r.tanks.map((t) => t.fuelType)).filter(Boolean))].sort(),
    [bunkerings]
  );

  const sorted = useMemo(() => {
    const base = [...bunkerings].sort((a, b) => {
  ...
  }, [bunkerings, dateFrom, dateTo, fuelType]);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          <IconDroplet size={15} stroke={2} style={{ color: "rgba(255,255,255,0.60)" }} />
          ...
          {hasFetched && (
            ...
            >
              {hasFilter ? `${sorted.length} / ${bunkerings.length}` : sorted.length}
            </span>
          )}
        </div>
        <button
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              className="text-[12px] font-semibold text-[#04111f] bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition-all"
            >
        ...
        <div className="flex-1 min-h-0 overflow-auto">
          {showSkeleton ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 py-20">
              ...
              >
                <IconDroplet size={26} stroke={1.4} style={{ color: "#0369a1" }} />
              </div>
              <div className="text-center">
                <p className="text-[14px] font-semibold text-[#04111f]">
                  {hasFilter ? "No records match the selected filters" : "No bunkering entries yet"}
                </p>
                <p className="text-[12.5px] text-slate-400 mt-1">
                  {hasFilter
                    ? "Try adjusting the date range or fuel type filter."
                    : <> Tap <span className="font-semibold text-[#0369a1]">New Bunkering</span> to add the first entry.</>}
                </p>
              </div>
            </div>
